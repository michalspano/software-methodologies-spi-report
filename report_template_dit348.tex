...
% TODO: examine the structure and the formatting of this section.

% A measurement plan contains:
...
Herein, a goal is written in the following manner: \textit{X)}, where
\textit{X} is an ordinal. A question follows the notation
\textbf{Q\textit{X.Y}}, where \textit{Y} is the question's ordinal. Similarly,
a metric follows a similar notation, notably, \textbf{M\textit{X.Z}}, where
\textit{Z} is the metric's ordinal. A metric, in this context, provides a
...
\begin{itemize}
  ...
  \item \textbf{M1.2}: A formulate in the following manner is to be used:
    ...
    sprint). In the context of a Lego Scrum workshop, it is difficult to keep
    ...
    was appended to a merge request (or a pull request) that was merged into
    the main branch of the project's repository. Nevertheless, for our case, we
...
\begin{itemize}
  ...
  \item \textbf{M2.1}:
    ...
    at the beginning of a sprint by all team members). For this example,
    let's say that one story point represents 5 minutes. A randomly chosen team
    member determines the total number of story points completed in a sprint, say
    $s$, and then collects the data from the team members regarding the time
    spent working on the user stories [in minutes], say $t$. The team members
    would be previously instructed on how to track the time spent working during a
    sprint with the help of an appropriate tool such as Toggl (or any other
    tool that is deemed suitable). The chosen team member would then perform a
    simple calculation on their local machine in the following way:
    $\frac{s}{t}$. In this case, the ideal ratio is $\approx \frac{1}{5}$. A
    threshold interval can be defined as $[\frac{1}{5} - \epsilon, \frac{1}{5}
    + \epsilon]$, where $\epsilon$ is some small number, say $0.1$. If the
    ratio falls within the threshold interval, then the story points are
    considered to be accurate -- this ratio can be altered to fit the team's
    needs and can be derived from the team's experience in the previous sprints
    and/or projects. The results, for a future reference, can be stored in a
    primary communication channel (e.g., Slack, Discord, etc.).
  ...
  \item \textbf{M2.2}: 
    ...
    basis, i.e., they should complete the same number of story points. Measure
    the number of story points that each team member completes during a sprint,
    and assign them to variables $x_1, x_2, \dots, x_n$, where $n$ is the size
    of the team (it would be the responsibility of each team members to keep
    track of and report the number of story points they completed). At the end
    of the sprint, a randomly chosen team member collects all data entries and
    performs a calculation on their local machine in the following manner:
    $\beta = \sum_{i=1}^{n} |x_i - m|$. Herein, $\beta$ is an arbitrary value
    that represents the workload distribution among the team members. The
    closer $\beta$ is to $0$, the more evenly distributed the workload is. In
    that essense, similarly to the previous metric, a threshold interval can be
    defined, and an appropriate $\epsilon$ can be chosen (this is to be
    determined by the team members).
\end{itemize}

Lastly, 3) is accompanied with the following questions and metrics:

% TODO: check the wording; apply all principles to construct a measurement plan.
\begin{itemize}
  \item \textbf{Q3.1}: How effective are the cross-team communication efforts?
  \item \textbf{M2.1}: An averaged satisfaction score that is obtained from
    the team members and all other parties involved in any cross-team meetings.
    A traditional Likert scale from $1 \to 5$ can be used to obtain the scores,
    where $1$ corresponds to \textit{"Strongly dissatisfied"} and $5$ corresponds
    to \textit{"Strongly satisfied"}. Ideally, the team should strive to obtain
    a score of $\approx 5$.
  \item \textbf{Q2.2}: How well do contributions from all teams function together?
  \item \textbf{M2.2}: The number of integration problems occurring during the
    project (or during a sprint, if a more granular approach is desired).
\end{itemize}

% TODO: to define at least two concrete improvement steps per goal; USE A4!
% Careful to reference the corresponding literature when you - use literature.
...
% what has been observed in the literature. Relate these challenges to
% empirical studies that have been published and to your personal experience.
